...
// Held back unconditionally (checked before evolution status, not just as a
// "no evolution" fallback) - PokeAPI doesn't flag any of these
// is_legendary/is_mythical, so the filter below them doesn't catch them:
//  - 8 of the 10 Ultra Beasts (793-799, 805, 806, all non-evolving): a direct
//    baseAttack+baseDefense check confirmed they're literally the top 3
//    highest-stat single-stage species in the entire 1-8세대 range (Stakataka
//    342, Kartana 312, Buzzwole 278 vs a ~167 pool average) - ordinary odds
//    would make them the best possible draw in the game.
//  - Poipole (803), the 9th Ultra Beast: its own stats are actually below
//    average (73/67), so it's excluded for the "same family" lore reason
//    rather than a stat one - being consistent about the category matters
//    more here than the number on this particular member. Its evolution
//    Naganadel (804) never reaches this function as a candidate id anyway,
//    since it isn't a first-stage species (has evolves_from_species set).
...
const EXCLUDED_SPECIES_IDS = new Set([213, 793, 794, 795, 796, 797, 798, 799, 803, 805, 806]);
...
// species with NO evolution at all is let in too (unless EXCLUDED_SPECIES_IDS
// above holds it back) - those adopt as permanent final forms (/펫정보 already
// renders "더 이상 진화하지 않아요" for this shape of pet).
async function getEligibleBaseSpecies(id) {
  ...
  if (EXCLUDED_SPECIES_IDS.has(id)) return null;

  return (await getFollowingEvolution(id)) ?? { options: [], minLevel: null, rare: false };
}
