...
module.exports = {
  ...
  execute(message, args, client, mongoose, guild) {
    if (args[0] != null) {
      ...
      const charZero = mbti.charAt(0);
      const charOne = mbti.charAt(1);
      const charTwo = mbti.charAt(2);
      const charThree = mbti.charAt(3);
        ...
        if (charOne == "S" && "N") {
          if (charTwo == "F" && "T") {
            if (charThree == "J" && "P") {
                  ...
                  .then(res => {
                    ...
                    if (res == null) {
                      mbtiObj
                        ...
                        let role = message.guild.roles.cache.find(r => r.name == mbti);
              ...
              return message.channel.send(
                "MBTI가 제대로 입력되지 않았습니다. (Ex. ISFP ENTP)"
              );
            }
          } else {
            return message.channel.send(
