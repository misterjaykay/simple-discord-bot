  ...
  async execute(interaction) {
    ...
    if (sub === "지정") {
      ...
      if (channel.id !== interaction.channelId) {
        try {
          ...
          await targetChannel.send(
            "🐾 이제부터 이 채널에서만 펫 관련 명령어(입양/정보/밥주기/놀아주기/진화/이름변경/파양/슬롯/활성화/보관/꺼내기/보관함/대전)를 사용할 수 있어요!"
          );
        } catch (err) {
      ...
      return interaction.reply(
        `앞으로 펫 관련 명령어(입양/정보/밥주기/놀아주기/진화/이름변경/파양/슬롯/활성화/보관/꺼내기/보관함/대전)는 ${channel}에서만 사용할 수 있어요.`
      );
    }
